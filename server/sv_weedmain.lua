if Config.EnableWeed then
    ...
    Citizen.CreateThread(function()
        while true do
            ...
            if PlantsLoaded then
                TriggerClientEvent('jjsslowmohud:weed:client:updateWeedData',
                                   -1, Config.Plants)
            end
        end
    end)
    ...
    ESX.RegisterUsableItem('weed_og-kush_seed', function(source)
        ...
        TriggerClientEvent('jjsslowmohud:weed:client:plantNewSeed', src,
                           'og_kush')
    ...
    AddEventHandler('jjsslowmohud:weed:server:saveWeedPlant', function(data)
        ...
        MySQL.Async.execute(
            'INSERT INTO weed_plants (properties) VALUES (@properties)',
            {['@properties'] = data}, function() end)
    end)

    RegisterServerEvent('jjsslowmohud:server:checkPlayerHasThisItem')
    AddEventHandler('jjsslowmohud:server:checkPlayerHasThisItem',
                    function(item, cb)
        local src = source
        local xPlayer = ESX.GetPlayerFromId(src)
        ...
        if xPlayer.getInventoryItem(item).count > 0 then
            ...
            TriggerClientEvent('jjsslowmohud:weed:client:notify', src,
                               'You are missing ' .. item)
        end
    end)
    ...
    AddEventHandler('jjsslowmohud:weed:server:plantNewSeed',
        ...
        for y = 1, #Config.YieldRewards do
            if type == Config.YieldRewards[y].type then
                lable = tostring(Config.YieldRewards[y].label)
            end
            y = y + 1
        end
        local SeedData = {
            ...
            light = Config.StartingLight,
            air = Config.StartingAir,
            temp = Config.StartingTemp,
            humidity = Config.StartingHumidity,
            ph = Config.StartingPH,
            tds = Config.StartingTDS,
        ...
        if PlantCount >= Config.MaxPlantCount then
            TriggerClientEvent('jjsslowmohud:weed:client:notify', src,
                               'You already have ' .. Config.MaxPlantCount ..
                                   ' plants down')
        else
    ...
    AddEventHandler('jjsslowmohud:weed:plantHasBeenHarvested', function(plantId)
        for k, v in pairs(Config.Plants) do
            if v.id == plantId then v.beingHarvested = true end
        end
    ...
    AddEventHandler('jjsslowmohud:weed:destroyPlant', function(plantId)
        ...
        TriggerClientEvent('jjsslowmohud:weed:client:removeWeedObject', -1,
                           plantId)
        TriggerEvent('jjsslowmohud:weed:server:weedPlantRemoved', plantId)
        TriggerEvent('jjsslowmohud:weed:server:updatePlants')

        TriggerClientEvent('jjsslowmohud:weed:client:notify', src,
                           'You destroy the weed plant')
    end)
    ...
    AddEventHandler('jjsslowmohud:weed:harvestWeed', function(plantId)
            ...
            if v.id == plantId then
                for y = 1, #Config.YieldRewards do
                    if v.type == Config.YieldRewards[y].type then
                        ...
                        amount = math.random(Config.YieldRewards[y].rewardMin,
                                             Config.YieldRewards[y].rewardMax)
        ...
        if hasFound then
            ...
            if label ~= nil then
                TriggerClientEvent('jjsslowmohud:weed:client:notify', src,
                                   'You harvest x' .. amount .. ' ' .. label)
            end
            xPlayer.addInventoryItem(item, amount)
            if goodQuality then
                if math.random(1, 10) > 3 then
                    local seed = math.random(1, #Config.GoodSeedRewards)
                    xPlayer.addInventoryItem(Config.GoodSeedRewards[seed],
                                             math.random(2, 4))
                end
            else
                xPlayer.addInventoryItem(Config.BadSeedRewards,
                                         math.random(1, 2))
            end
        else
            print('did not find')
    ...
    AddEventHandler('jjsslowmohud:weed:server:updatePlants', function()
        TriggerClientEvent('jjsslowmohud:weed:client:updateWeedData', -1,
                           Config.Plants)
        TriggerClientEvent('jjsslowmohud:weed:client:updateWeedobjData', -1,
                           Config.PlantsObj)
    end)
    ...
    AddEventHandler('jjsslowmohud:weed:server:waterPlant', function(plantId)
        ...
        for k, v in pairs(Config.Plants) do
            if v.id == plantId then
                Config.Plants[k].thirst =
                    Config.Plants[k].thirst +
                        math.random(Config.ThirstIncrease.min,
                                    Config.ThirstIncrease.max)
                Config.Plants[k].tds = Config.Plants[k].tds -
                                           (math.random(Config.TDSIncrease.min,
                                                        Config.TDSIncrease.max) /
                                               5)
    ...
    AddEventHandler('jjsslowmohud:weed:server:feedPlant', function(plantId)
        ...
        for k, v in pairs(Config.Plants) do
            if v.id == plantId then
                Config.Plants[k].hunger =
                    Config.Plants[k].hunger +
                        math.random(Config.HungerIncrease.min,
                                    Config.HungerIncrease.max)
                Config.Plants[k].tds = Config.Plants[k].tds +
                                           math.random(Config.TDSIncrease.min,
                                                       Config.TDSIncrease.max)
                Config.Plants[k].ph = Config.Plants[k].ph - 0.3
    ...
    AddEventHandler('jjsslowmohud:weed:server:updateWeedPlant',
                    function(id, data)
        ...
        if result[1] then
            for i = 1, #result do
                ...
                if plantData.id == id then
                    ...
                    MySQL.Async.execute(
                        'UPDATE weed_plants SET properties = @properties WHERE id = @id',
                        {['@properties'] = newData, ['@id'] = result[i].id},
                        function() end)
                end
            end
        end
    ...
    AddEventHandler('jjsslowmohud:weed:server:weedPlantRemoved',
                    function(plantId)
        local result = MySQL.Sync.fetchAll('SELECT * FROM weed_plants')
        ...
        if result then
            for i = 1, #result do
                ...
                if plantData.id == plantId then
                    ...
                    MySQL.Async.execute(
                        'DELETE FROM weed_plants WHERE id = @id',
                        {['@id'] = result[i].id})
    ...
    AddEventHandler('jjsslowmohud:weed:server:weedPlantGrow',
                    function(plantId) end)
            ...
            for i = 1, #Config.Plants do
                ...
                if Config.Plants[i].growth < 100 then
                    if Config.Plants[i].grace then
                        ...
                        local light = Config.Plants[i].light or
                                          Config.StartingLight
                        local temp = Config.Plants[i].temp or
                                         Config.StartingTemp
                        ...
                        waterusage = round(
                                         (light + temp + air) * plantage / 100,
                                         1)
                        tdsusage = round(
                                       (light + temp + air) * plantage / ph / 10,
                                       1)
                        saltbuildup = round(waterusage / tds * waterage, 2)
                        phflux = 0.01 + (saltbuildup / tdsusage) +
                                     (0.01 + plantage - waterage) / 50
                        ph = round(ph - (phflux / 25), 3)
                        phflux = round(phflux, 3)
                        tds = tds - (phflux / 10)
                        tds = round(tds, 2)
                        if (tds > ((plantage + 7 * 7) * 3)) then
                            tds = math.random(1900, 2900)
                            ph = math.random(3.3, 4.2)
                        end

                        healthflux = ((light - plantage) + (temp - plantage) +
                                         (air - plantage) + (phflux * -2))
                        healthflux = round(healthflux * (plantage / 100), 4)
                        if healthflux < -3 then
                            healthflux = -3
                        end
                        if healthflux > 2 then
                            healthflux = 2
                        end
                        if saltbuildup > 8 then
                            saltbuildup = 8
                        end
                        health = round(health + healthflux, 1)
                        if health < 0 then health = 0 end
                        if health > 150 then health = 150 end
                        if ph < 4.0 then ph = 4.0 end
                        if ph > 8.0 then ph = 8.0 end
                        if tds < 0 then tds = 0 end
                        ...
                        if Config.debug then
                            print("saltbuildup " .. saltbuildup)
                        end
                        if Config.debug then
                            print("tdsusage " .. tdsusage)
                        end
                        if Config.debug then
                            print("waterusage " .. waterusage)
                        end
                        if Config.debug then
                            print("health " .. health)
                        end

                        Config.Plants[i].thirst = round(
                                                      Config.Plants[i].thirst -
                                                          math.random(
                                                              Config.Degrade.min,
                                                              Config.Degrade.max) /
                                                          8, 1)
                        Config.Plants[i].hunger = round(
                                                      Config.Plants[i].hunger -
                                                          math.random(
                                                              Config.Degrade.min,
                                                              Config.Degrade.max) /
                                                          5, 1)
                        Config.Plants[i].growth = round(
                                                      Config.Plants[i].growth +
                                                          (Config.GrowthIncrease *
                                                              (health / 100)), 1)
                        Config.Plants[i].quality =
                            round(Config.Plants[i].quality * (health / 100), 1)
                        if Config.Plants[i].growth > 100 then
                        ...
                        if Config.Plants[i].thirst < 75 or
                            Config.Plants[i].hunger < 75 then
                            Config.Plants[i].quality =
                                Config.Plants[i].quality -
                                    math.random(Config.QualityDegrade.min,
                                                Config.QualityDegrade.max) / 10
                        end

                        if Config.Plants[i].stage == 1 and
                            Config.Plants[i].growth >= 55 then
                            Config.Plants[i].stage = 2
                        elseif Config.Plants[i].stage == 2 and
                            Config.Plants[i].growth >= 90 then
                ...
                TriggerClientEvent('jjsslowmohud:plantstats', -1,
                                   Config.Plants[i])

                TriggerEvent('jjsslowmohud:weed:server:updateWeedPlant',
                             Config.Plants[i].id, Config.Plants[i])
            end
            TriggerEvent('jjsslowmohud:weed:server:updatePlants')
        end
    end)
    ...
    AddEventHandler('jjsslowmohud:weed:server:topPlant', function(plantId)
        ...
        for k, v in pairs(Config.Plants) do
            if v.id == plantId then Config.Plants[k].top = true end
        end
    ...
    AddEventHandler('jjsslowmohud:weed:server:fimPlant', function(plantId)
        ...
        for k, v in pairs(Config.Plants) do
            if v.id == plantId then Config.Plants[k].fim = true end
        end
    ...
    AddEventHandler('jjsslowmohud:weed:server:feminizePlant', function(plantId)
        ...
        for k, v in pairs(Config.Plants) do
            if v.id == plantId then Config.Plants[k].fem = true end
        end
    ...
    AddEventHandler('jjsslowmohud:weed:server:lstPlant', function(plantId)
        ...
        for k, v in pairs(Config.Plants) do
            if v.id == plantId then Config.Plants[k].lst = true end
        end
    ...
    AddEventHandler('jjsslowmohud:weed:server:revegPlant', function(plantId)
        ...
        for k, v in pairs(Config.Plants) do
            if v.id == plantId then Config.Plants[k].reveg = true end
        end
    ...
    AddEventHandler('jjsslowmohud:weed:server:clonePlant', function(plantId)
        ...
        for k, v in pairs(Config.Plants) do
            if v.id == plantId then Config.Plants[k].clone = true end
        end

    end)
    RegisterServerEvent('jjsslowmohud:weed:server:monstercropPlant')
    AddEventHandler('jjsslowmohud:weed:server:monstercropPlant',
                    function(plantId)
        local src = source
        local xPlayer = ESX.GetPlayerFromId(src)
    ...
    AddEventHandler('jjsslowmohud:weed:server:supercropPlant', function(plantId)
        ...
        for k, v in pairs(Config.Plants) do
            if v.id == plantId then Config.Plants[k].supercrop = true end
        end
    ...
    AddEventHandler('jjsslowmohud:weed:server:checkPH', function(plantId)
        ...
        for k, v in pairs(Config.Plants) do
            if v.id == plantId then return Config.Plants[k].ph end
        end
    ...
    AddEventHandler('jjsslowmohud:weed:server:checkTDS', function(plantId)
        ...
        for k, v in pairs(Config.Plants) do
            if v.id == plantId then return Config.Plants[k].tds end
        end
    ...
    AddEventHandler('jjsslowmohud:weed:server:checkLight', function(plantId)
        ...
        for k, v in pairs(Config.Plants) do
            if v.id == plantId then return Config.Plants[k].lights end
        end
    ...
    AddEventHandler('jjsslowmohud:weed:server:checkHumidity', function(plantId)
        ...
        for k, v in pairs(Config.Plants) do
            if v.id == plantId then return Config.Plants[k].humidity end
        end
    ...
    AddEventHandler('jjsslowmohud:weed:server:checkTemp', function(plantId)
        ...
        for k, v in pairs(Config.Plants) do
            if v.id == plantId then return Config.Plants[k].temp end
        end
    ...
    AddEventHandler('jjsslowmohud:weed:server:checkAir', function(plantId)
        ...
        for k, v in pairs(Config.Plants) do
            if v.id == plantId then return Config.Plants[k].airflow end
        end
